...
Tested against **EmDash `0.14.x` and later** (`emdash` peer **`>=0.14.0`** on `@emplugins/emdash-plugin-md-draft`). Ingest uses `POST` with `Content-Type: application/json` and `{ "markdown": "..." }` because EmDash’s plugin route host parses JSON before the handler runs (see [`docs/operator-runbook.md`](./docs/operator-runbook.md)). Multi-locale sites may set optional YAML `locale` and `translationOf` in the markdown frontmatter.
